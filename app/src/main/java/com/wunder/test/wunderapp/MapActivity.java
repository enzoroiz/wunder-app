...
public class MapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        for (int i = 0; i < cars.size(); i++) {
            Car car = cars.get(i);
            map.addMarker(new MarkerOptions().position(car.getLocation()).title(car.getName()));
        }

        Toast.makeText(this, selectedCarId + " tes", Toast.LENGTH_SHORT).show();
        if (selectedCar != null) {
    ...
    public void getSelectedCar(int carId) {
        if (carId == -1) {
            return;
        }
